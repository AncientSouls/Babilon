...
export const createValidate = (rules: IRules): IValidator => (last, flow) => {
  ...
  if (last.exp[0] === 'select') flow.validateMemory.selects.push(last);
  if (last.exp[0] === 'alias') flow.validateMemory.aliases.push(last);
