...
exports.isType = (last, rules, exp, arg, i) => {
    if (arg[0] === ':') {
        if (exports.isTypes(last, rules, rules.types[_.trimStart(arg, ':')], exp, i)) {
            return true;
        }
    }
    else if (arg[0] === '!') {
            ...
            if (rule.all) {
                ...
                for (e = 1; e < last.exp.length; e++) {
                    if (!exports.isTypes(last, rules, rule.all, last.exp[e], e - 1)) {
                        return babilon_1.error(last.exp[0], `arg [${e - 1}] is not any of all [${rule.all}]`, flow);
                    }
